...
def disparity_to_depth(disparity_p, depth_p, calib, scale_factor):
        disparity = sio.load_subpix_png(disparity_p, scale_factor)
        img_3d = cvt.disparity_to_depthmap(disparity, calib['Q'])
        sio.save_subpix_png(depth_p, img_3d, scale_factor)
...
def main():
    ...
    # use this for SERV-CT
    with Pool(args.jobs) as pool:
        pool.starmap(disparity_to_depth, tqdm(zip(disparity_paths,
                                        depth_paths,
                                        repeat(calib),
                                        repeat(args.scale_factor)), total=len(disparity_paths)))
    return 0   
