...
void generateDisparityMapGPU(char leftFileName[], char rightFileName[], char dispMapSaveName[], char costMapSaveName[], int w, int h, int winSize, int avgwinSize, int maxDisparity){
	...
	cout << "Processing Time of " << leftFileName << ": " << duration.count() << "ms." << endl;
	//cleanup arrays
	delete[] leftimg;
	delete[] rightimg;
	delete[] disparityMap;
...
int main(int argc, char *argv[] ) {
	...
	*/
	...
	int numSeqMin = 1; //starting sequence ID: numSeqMin >= 0 && numSeqMin <= 193/194
	int numSeqMax = 4; //ending sequence ID: numSeqMax >= 0 && numSeqMax <= 193/194, && numSeqMax >= numSeqMin
	...
	double baseline = 0.0055; //stereo baseline
	int maxDisparity = 194;
	...
	for (int numSeq = numSeqMin; numSeq <= numSeqMax; numSeq++){
		...
		if (numSeq == 31 || numSeq == 114){ //missing data TRAINING KITTI
			continue;
		}
		else{
			...
			//	sprintf(leftFileName, "./training_image_0/%03d.pgm", numSeq);
			//	sprintf(rightFileName, "./training_image_1/%03d.pgm", numSeq);
			...
			//		sprintf(costMapSaveName, "./costmap_GPU_%s%dx%d_%dx%d/%03d.pgm.bin", stereoMethod, winSize, winSize, avgwinSize, avgwinSize, numSeq);
			//		sprintf(dispMapSaveName, "./disparity_GPU_%s%dx%d_%dx%d/%03d.pgm.pgm", stereoMethod, winSize, winSize, avgwinSize, avgwinSize, numSeq);
			...
			if (useGPU) {
				...
				sprintf(costMapSaveName, "./costmap_CPU_%s%dx%d_%dx%d/%03d.pgm.bin", stereoMethod, winSize, winSize, avgwinSize, avgwinSize, numSeq);
				sprintf(dispMapSaveName, "./disparity_CPU_%s%dx%d_%dx%d/%03d.pgm.pgm", stereoMethod, winSize, winSize, avgwinSize, avgwinSize, numSeq);
				generateDisparityMap(leftFileName, rightFileName, dispMapSaveName, costMapSaveName, w, h, winSize, avgwinSize, maxDisparity);
			}
		}
	}
